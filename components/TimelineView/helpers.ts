 ...
 */
export const daydiff = (first: Date, second: Date): number =>
  Math.round(second.valueOf() - first.valueOf());
 ...
 */
export const zip = (rows: Array<Array<any>>): Array<any> =>
 ...
 */
export const dateDistanceExtremes = (dates: Array<any>): { min: number; max: number } => {
 ...
 * that set of dates.
 *
 * @param {Array<Date>} dates the array containing dates the dates
...
export const cummulativeSeperation = (
  ...
  const paddingDiff = (maxEventPadding ?? 4) - (minEventPadding ?? 1);
  ...
  for (let index = 1; index < distances.length; index += 1) {
    ...
    const seperation =
      datesDiff === 0
        ? maxEventPadding ?? 4
        : Math.round(
            ((distance - dateExtremes.min) * paddingDiff) / datesDiff + (minEventPadding ?? 1)
          );
