...
import { calculateSpacing, SpacingType } from "@components/Timeline/utils";
...
const TimelineGuideO: React.FC<TimelineGuideDProps> = ({
  ...
  useEffect(() => {
    ...
    guideRef.current
      ?.querySelectorAll("a")
      [currentIndex].scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  return (
    <TimelineGuide
      ...
      <div>
        {itemSpacing.map((v) => {
          switch (v.type) {
            case SpacingType.Link:
              return (
                <TimelineGuideItem key={v.key}>
                  <a href={`#${v.id}`}>
            ...
            case SpacingType.Labeled:
            case SpacingType.Unlabeled:
              return <div key={v.key} style={{ height: v.size ?? "auto" }} />;
            default:
              return null;
          }
        })}
